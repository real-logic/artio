...
public class SenderTargetAndSubSessionIdStrategyTest
{
        ...
        final Set<Object> compositeKeys =
            IDS.stream().flatMap((sender) ->
                IDS.stream().flatMap((senderSub) ->
                    IDS.stream().map(
                        (target) -> strategy.onLogon(sender, senderSub, null, target))))
            .collect(toSet());
    ...
    public void theSameIdIsEqual()
    {
        IDS.forEach((sender) ->
            IDS.forEach((senderSub) ->
                IDS.forEach((target) ->
                {
                    final Object first = strategy.onLogon(sender, senderSub, null, target);
                    final Object second = strategy.onLogon(sender, senderSub, null, target);
    ...
    public void savesAndLoadsACompositeKey()
    {
        ...
        final CompositeKey key = strategy.onLogon("SIGMAX", "LEH_LZJ02", null, "ABC_DEFG04");
